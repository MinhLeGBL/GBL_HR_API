...
class TestPeriodWindows:
    def test_week_window_is_the_last_COMPLETE_week_not_week_to_date(self):
        ...
        assert w['WTD'][0] == (date(2026, 8, 31), date(2026, 9, 6))
    ...
    def test_sunday_week_start_shifts_the_whole_window(self):
        ...
        assert w['WTD'][0] == (date(2026, 8, 30), date(2026, 9, 5))
    ...
    def test_week_window_is_always_exactly_seven_days(self, as_of):
        (cf, ct), (pf, pt) = pr.period_windows(as_of)['WTD']
        assert (ct - cf).days == 6
        assert (pt - pf).days == 6
    ...
    def test_week_window_never_reaches_as_of(self, as_of):
        (_cf, ct), _prior = pr.period_windows(as_of)['WTD']
        assert ct < as_of
    ...
    def test_prior_wtd_is_iso_week_aligned_not_calendar_date_aligned(self):
        w = pr.period_windows(date(2026, 9, 9))
        # Current is 2026-W36 (Mon 08-31..Sun 09-06); 2025-W36 is 09-01..09-07.
        # A calendar-date shift would have given 2025-08-31..09-06.
        assert w['WTD'][1] == (date(2025, 9, 1), date(2025, 9, 7))

    def test_prior_wtd_preserves_weekdays(self):
        (cur_from, cur_to), (pri_from, pri_to) = pr.period_windows(date(2026, 9, 9))['WTD']
        assert cur_from.weekday() == pri_from.weekday()
        assert cur_to.weekday() == pri_to.weekday()

    def test_prior_wtd_has_the_same_week_number(self):
        (cur_from, _), (pri_from, _) = pr.period_windows(date(2026, 9, 9))['WTD']
        assert cur_from.isocalendar().week == pri_from.isocalendar().week

    def test_both_wtd_sides_span_the_same_number_of_days(self):
        (cf, ct), (pf, pt) = pr.period_windows(date(2026, 9, 9))['WTD']
        assert (ct - cf) == (pt - pf)


class TestIsoWeekHelpers:
    @pytest.mark.parametrize('year,weeks', [
        (2024, 52), (2025, 52), (2026, 53), (2027, 52), (2020, 53), (2015, 53),
    ])
    def test_iso_weeks_in_year(self, year, weeks):
        assert pr.iso_weeks_in_year(year) == weeks
...
class TestPriorWeekWindow:
    def test_same_week_number_one_year_earlier(self):
        start, end = pr.prior_week_window(date(2026, 9, 7), date(2026, 9, 9))
        assert (start, end) == (date(2025, 9, 8), date(2025, 9, 10))
        assert start.isocalendar().week == 37

    def test_span_length_is_preserved(self):
        start, end = pr.prior_week_window(date(2026, 9, 7), date(2026, 9, 9))
        assert (end - start).days == 2

    def test_weekday_position_is_preserved_for_a_midweek_start(self):
        # A Wednesday start maps to the prior year's Wednesday.
        start, _ = pr.prior_week_window(date(2026, 9, 9), date(2026, 9, 9))
        assert start.weekday() == date(2026, 9, 9).weekday()

    def test_week_53_clamps_to_the_prior_years_final_week(self):
        # 2026 has 53 ISO weeks; 2025 has only 52, so W53 clamps to W52.
        w53_monday = date.fromisocalendar(2026, 53, 1)
        start, _end = pr.prior_week_window(w53_monday, w53_monday)
        iso = start.isocalendar()
        assert (iso.year, iso.week) == (2025, 52)
...
class TestFetchWindowCoverage:
    ...
    def test_early_january_reaches_back_into_the_previous_december(self):
        # Run on 1 Jan 2026: the last complete week is 2025-W52, whose
        # prior-year counterpart (2024-W52) starts 2024-12-23 -- well before
        # the YTD prior window's 2025-01-01.
        windows = pr.period_windows(date(2026, 1, 1))
        fetch_from = min(start for w in windows.values() for start, _end in w)
        assert fetch_from == date(2024, 12, 23)
...
class TestMetricColumns:
    ...
    def test_money_columns_use_a_millions_format(self):
        ...
        assert {key for _l, key, _u in money} == {
            ...
            'total_discount_value', 'returns_value',
        }
    ...
    def test_aggregate_totals_stay_whole_millions(self):
        by_key = {key: unit for _l, key, unit in pr.METRIC_COLUMNS}
        for key in ('total_sales', 'total_discount_value', 'returns_value'):
