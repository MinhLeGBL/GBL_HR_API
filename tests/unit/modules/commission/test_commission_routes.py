...
def _batch_payload(**overrides):
    """Return a valid payload for the batch/calculate endpoint."""
    payload = {
        'year': 2025,
        'month': 6,
...
# ===========================================================================
# 2. POST /api/v1/commission/batch/calculate
# ===========================================================================

class TestCalculateBatchCommission:
    """Tests for the batch commission calculation endpoint."""

    URL = '/api/v1/commission/batch/calculate'

    @patch('app.modules.commission.routes.CommissionService')
    def test_success(self, MockCommissionService, client):
        mock_service = MockCommissionService.return_value
        mock_service.calculate_batch_store_commissions.return_value = {
            'success': True,
            'data': {'total_commission': 15000000},
        }

        resp = client.post(self.URL, json=_batch_payload())

        assert resp.status_code == 200
        body = resp.get_json()
        assert body['success'] is True
        mock_service.calculate_batch_store_commissions.assert_called_once()
    ...
    def test_missing_stores(self, client):
        payload = _batch_payload()
        del payload['stores']
        resp = client.post(self.URL, json=payload)
    ...
    def test_invalid_month(self, client):
        resp = client.post(self.URL, json=_batch_payload(month=13))

        assert resp.status_code == 400
        assert 'month must be between 1 and 12' in resp.get_json()['error']

    def test_empty_employees(self, client):
        resp = client.post(self.URL, json=_batch_payload(employees=[]))

        assert resp.status_code == 400
        assert 'non-empty list' in resp.get_json()['error']

    def test_empty_stores(self, client):
        resp = client.post(self.URL, json=_batch_payload(stores=[]))
    ...
    def test_employee_missing_employee_code(self, client):
        ...
        resp = client.post(self.URL, json=_batch_payload(employees=[bad_emp]))

        assert resp.status_code == 400
        body = resp.get_json()
        assert 'employee_code' in body['error']

    def test_employee_missing_seniority(self, client):
        bad_emp = {
            'employee_code': 'EMP001',
            'employee_name': 'Alice',
            'store_code': 'RHN',
        ...
        assert resp.status_code == 400
        body = resp.get_json()
        assert 'seniority' in body['error']

    def test_store_missing_store_code(self, client):
        bad_store = {
            'target_revenue': 500000000,
            'target_fp_ratio': 0.6,
        }
        resp = client.post(self.URL, json=_batch_payload(stores=[bad_store]))

        assert resp.status_code == 400
        body = resp.get_json()
        assert 'store_code' in body['error']

    def test_store_missing_target_revenue(self, client):
        bad_store = {
            'store_code': 'RHN',
            'target_fp_ratio': 0.6,
        }
        ...
        assert resp.status_code == 400
        body = resp.get_json()
        assert 'target_revenue' in body['error']

    def test_store_missing_target_fp_ratio(self, client):
        bad_store = {
            'store_code': 'RHN',
            'target_revenue': 500000000,
        }
        ...
        assert resp.status_code == 400
        body = resp.get_json()
        assert 'target_fp_ratio' in body['error']
    ...
    def test_invalid_fp_ratio_too_high(self, client):
        ...
        resp = client.post(self.URL, json=_batch_payload(stores=stores))

        assert resp.status_code == 400
        body = resp.get_json()
        assert 'target_fp_ratio must be between 0.0 and 1.0' in body['error']
    ...
    def test_exception_returns_500(self, MockCommissionService, client):
        mock_service = MockCommissionService.return_value
        mock_service.calculate_batch_store_commissions.side_effect = RuntimeError(
            'Unexpected error'
        )
...
# ===========================================================================
# 4. GET /api/v1/commission/store/calculate-from-sheet
# ===========================================================================

class TestCalculateStoreCommissionFromSheet:
    """Tests for the Google Sheets-based commission calculation endpoint."""

    URL = '/api/v1/commission/store/calculate-from-sheet'

    def _build_sheet_data(self):
        """Return a realistic sheet_data dict as returned by GoogleSheetsService."""
        return {
            'query_period': {
                'from_date': '2025-06-01 00:00:00',
                'to_date': '2025-06-30 23:59:59',
    ...
    def _build_store_commission_result(self):
        return {
            'store_code': 'RHN',
            'achievement_pct': 1.05,
            'actual_fp_ratio': 0.65,
    ...
    def _build_combined_df(self):
        return pd.DataFrame([{
            ...
            'full_name': 'Alice',
            'store_code': 'RHN',
            'total_handout_commission': 15000000,
        }])

    @patch('app.modules.commission.routes.pd')
    @patch('app.modules.commission.routes.GoogleSheetsService')
    @patch('app.modules.commission.routes.CommissionService')
    def test_success_with_spreadsheet_title(
        self, MockCommissionService, MockGoogleSheetsService, mock_pd, client
    ):
        mock_sheets = MockGoogleSheetsService.return_value
        mock_commission = MockCommissionService.return_value

        # find_spreadsheet_by_title returns an ID
        mock_sheets.find_spreadsheet_by_title.return_value = 'sheet-id-123'
        mock_sheets.get_sheet_data.return_value = self._build_sheet_data()

        store_result = self._build_store_commission_result()
        mock_commission.calculate_store_commission_v2.return_value = store_result
        mock_commission.calculate_personal_commissions.return_value = MagicMock()

        combined_df = self._build_combined_df()
        mock_commission.calculate_combined_commission.return_value = combined_df

        # pd.concat should return the combined df
        mock_pd.concat.return_value = combined_df

        resp = client.get(
            self.URL, query_string={'spreadsheet_title': 'Sales Jun 2025'}
        )
        ...
        assert body['data']['total_stores'] == 1
        assert body['data']['total_employees'] == 1
        assert body['data']['total_commission'] == 15000000
        assert len(body['data']['stores']) == 1
        assert body['data']['stores'][0]['store_code'] == 'RHN'

        mock_sheets.find_spreadsheet_by_title.assert_called_once_with('Sales Jun 2025')
        mock_sheets.clear_output_ranges.assert_called_once()
        mock_sheets.upload_commission_results.assert_called_once()

    @patch('app.modules.commission.routes.pd')
    @patch('app.modules.commission.routes.GoogleSheetsService')
    @patch('app.modules.commission.routes.CommissionService')
    def test_success_with_spreadsheet_id(
        self, MockCommissionService, MockGoogleSheetsService, mock_pd, client
    ):
        mock_sheets = MockGoogleSheetsService.return_value
        mock_commission = MockCommissionService.return_value
        ...
        resp = client.get(
            self.URL, query_string={'spreadsheet_id': 'sheet-id-456'}
        )
        ...
        # Should NOT attempt to find by title
        mock_sheets.find_spreadsheet_by_title.assert_not_called()

    @patch('app.modules.commission.routes.GoogleSheetsService')
    @patch('app.modules.commission.routes.CommissionService')
    def test_missing_both_spreadsheet_params(
        self, MockCommissionService, MockGoogleSheetsService, client
    ):
        ...
        body = resp.get_json()
        assert body['success'] is False
        assert 'spreadsheet_title or spreadsheet_id' in body['error']
    ...
    def test_spreadsheet_not_found(
        ...
        mock_sheets = MockGoogleSheetsService.return_value
        mock_sheets.find_spreadsheet_by_title.side_effect = Exception(
            'Spreadsheet not found'
        )

        resp = client.get(
            self.URL, query_string={'spreadsheet_title': 'Nonexistent'}
        )

        assert resp.status_code == 404
        body = resp.get_json()
        assert body['success'] is False
        assert 'Could not find spreadsheet' in body['error']

    @patch('app.modules.commission.routes.pd')
    @patch('app.modules.commission.routes.GoogleSheetsService')
    @patch('app.modules.commission.routes.CommissionService')
    def test_no_employees_error(
        self, MockCommissionService, MockGoogleSheetsService, mock_pd, client
    ):
        """When all stores have no matching employees, all_combined_dfs is empty."""
        mock_sheets = MockGoogleSheetsService.return_value
        mock_sheets.get_sheet_data.return_value = {
            ...
            # All employees belong to a DIFFERENT store than the one listed
            'employees': [
                {
                    'employee_code': 'EMP001',
                    'store_code': 'HBT',
        ...
        resp = client.get(
            self.URL, query_string={'spreadsheet_id': 'sheet-id-789'}
        )

        assert resp.status_code == 400
        body = resp.get_json()
        assert body['success'] is False
        assert 'No employees found' in body['error']
    ...
    def test_value_error_returns_400(
        ...
        mock_sheets = MockGoogleSheetsService.return_value
        mock_sheets.get_sheet_data.side_effect = ValueError(
            'Invalid date format in sheet'
        )

        resp = client.get(
            self.URL, query_string={'spreadsheet_id': 'sheet-id-abc'}
        )
    ...
    def test_general_exception_returns_500(
        ...
        mock_sheets = MockGoogleSheetsService.return_value
        mock_sheets.get_sheet_data.side_effect = RuntimeError(
            'Google API quota exceeded'
        )

        resp = client.get(
            self.URL, query_string={'spreadsheet_id': 'sheet-id-def'}
        )

        assert resp.status_code == 500
        body = resp.get_json()
        assert body['success'] is False
        assert 'Google API quota exceeded' in body['error']
